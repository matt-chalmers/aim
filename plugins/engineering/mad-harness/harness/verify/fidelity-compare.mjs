// Side-by-side fidelity compare: handover component vs our route, one image.
// Usage: node compare.mjs <name> <handoverComp> <ourPathWithQuery> <width>
...
// --- configuration -----------------------------------------------------------
// Everything here was hard-coded to one application: its ports, its login route, its
// field labels, its root element, and a committed password. All of it is now read from
// the environment, and the credentials have NO default.
...
const LOGIN_PATH      = process.env.FIDELITY_LOGIN_PATH   || '';   // empty = no login step
const USER            = process.env.FIDELITY_USER         || '';
const PASS            = process.env.FIDELITY_PASS         || '';
const USER_LABEL      = process.env.FIDELITY_USER_LABEL   || 'Username or email';
const PASS_LABEL      = process.env.FIDELITY_PASS_LABEL   || 'Password';
const SUBMIT_RE       = new RegExp(process.env.FIDELITY_SUBMIT || 'sign in', 'i');
...
// our render (logged in)
const oc = await b.newContext({ viewport:{ width, height: 1000 }, deviceScaleFactor: 1 });
const op = await oc.newPage();
if (LOGIN_PATH) {
  // NO CREDENTIAL IS DEFAULTED HERE. A committed password is a bad thing to ship even
  // for a local seed — and `verifier-security` is instructed to fail exactly this
  // pattern elsewhere, so shipping one would make the harness contradict its own lens.
  if (!USER || !PASS) {
    console.error(
      'FIDELITY_USER and FIDELITY_PASS are required when FIDELITY_LOGIN_PATH is set.\n' +
      'Export them for a local seed account; never commit them.'
    );
    process.exit(2);
  }
  await op.goto(`${APP_URL}${LOGIN_PATH}`,{ waitUntil:'networkidle' });
  await op.getByLabel(USER_LABEL).fill(USER);
  await op.getByLabel(PASS_LABEL,{ exact:true }).fill(PASS);
  await op.getByRole('button',{ name:SUBMIT_RE }).click();
  await op.waitForURL(x=>!x.pathname.endsWith(LOGIN_PATH),{ timeout:15000 }).catch(()=>{});
}
...
// Normalise both panels to a common height, then join side by side.
