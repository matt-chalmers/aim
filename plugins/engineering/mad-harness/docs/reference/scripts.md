...
| `swarm/lens-gate.sh <task> <sha> [--branch <b>] [--lane] [--worker N] [--l4 auto\|always] [--suite gate\|skip] [--wave] [--dry-run] [--json]` | /swarm step 7 and /grind §9 as one call: the brief and the L4 trigger → the suite once where the change is (the branch's worktree) → L1–L3 and L4-on-trigger dispatched at once, L3 handed no diff path → one verdict parser (no `VERDICT:` line, a denial, a hang or a budget kill is NONE, never PASS) → unanimity → the `VERIFIED <sha>` note only on all-PASS. Exit 0 PASS · 1 FAIL with the route printed · 2 could not judge |
| `swarm/fanout.sh --jobs <file> [--cap N] [--timeout S] [--wave]` / `--detach` / `--wait <id> [--timeout 540]` | N commands at once (one per line, no shell), each with a timeout, every one answered; a job past its timeout is HUNG. `--detach` runs them under a supervisor in its own session and `--wait` exits 5 while any is still running — the headless orchestrator's one call instead of a sleep loop |
| `swarm/wave-plan.sh <lane> [n] [--parent <epic>] [--json] [--no-manifest]` | /swarm steps 2–3 as one call: the lane's ready set, top n by priority clamped to the lane cap and `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`, every candidate's resume point (MERGE needs no worker), shared existing paths and megafiles dropped with the reason, candidate branches merge-tree'd; the two judgement checks (shared vocabulary, new files) printed with their inputs. `--parent` opens the epic's wave manifest |
| `swarm/merge-wave.sh <branch>... [--lane\|--stack …] [--wave] [--dry-run]` | /swarm step 8 as one call: refs and a clean tree checked before the slot, slot-acquire, each branch merged FROM THE REF ascending by task id (a conflict aborted and left unmerged, never resolved), slot-release in a finally, the gate once on the merged result (one `run.sh` per stack, concurrently; nothing declared is not green), red attributed to the task that touched the failing path. Suggests the revert, never performs it |
| `swarm/wave-report.sh <epic>-w<n> [--json]` | /swarm step 10's four signals and the 8b accretion check, computed from the wave manifest and git, with direction against the previous wave and the telemetry payload ready to record |
| `swarm/breakers.sh <epic> [--max-waves 6]` | campaign-loop §4.5's seven circuit breakers as arithmetic over the wave manifests; each trip printed with its prescribed action. Exit 0 clear · 1 tripped (your call) · 2 hard park · 3 stop the run |
| `swarm/halt.sh assess` / `pause <epic>` / `release [<task>…] [--drop-uncommitted]` | /halt as one call per form: assess reads everything a halt looks at and changes nothing; pause keeps the claims and parks the epic; release preserves every worktree's work FIRST (a failure stops everything), then per task the resume point, `release --force`, a note naming where the work is, and the worktree removed only for REATTACH under `--drop-uncommitted`. Both writes end with the slot (released only when its holder is provably gone), prune, export, commit, push, `autosync on` |
| `swarm/commit.sh <task> -m "…" -- <paths>` | a worker's commit as one call: the index checked for paths it did not name BEFORE the slot is taken, the tracker's export refused, the slot taken, the paths staged explicitly, the commit made, the slot released in a finally. Replaces the five-step sequence both writers carried in prose (one with a nonexistent `slot-acquire release`) |
| `swarm/worker-prompt.sh <task> [--lane] [--worker N] [--extra <file>]` | a writer's prompt assembled from the task record, how to run and commit here, the epic's SPEC INDEX slice the task's `SURFACE:` line points at, the matching field-guide keys, and a fidelity task's defect list; `dispatch.sh --task-prompt` calls it itself |
| `swarm/epic-queue.sh [--json] [--no-leases]` | campaign-loop §1 and §2 as one call: every open epic P0→P3 then oldest, each EXCLUDED with its reason (a gate holds it, a `PARKED` note, a lease another machine holds) or triaged UNPLANNED/PARTIAL/READY with its dispatchable-on-entry count. `campaign.sh` reads it |
| `swarm/campaign-signals.sh <epic> [--outcome closed\|parked\|stopped] [--mode] [--record]` | campaign-loop §6's eight health signals, computed from the wave manifests, the dispatch telemetry, the epic's `ADEQUACY:`/`AUDIT:` notes and git; `--record` files them as a campaign event with the outcome the epic actually had |
| `tracker/tk.sh decisions --rank` | open decisions ranked by the work each unblocks — transitive dependents, epics parked on it — one walk of the DAG |
| `swarm/plan-epic.sh <epic> [--mode auto\|interactive] [--from <stage>] [--triage …] [--reset] [--no-push] [--detach]` / `--wait <run-id> [--timeout 540]` | campaign-loop §3 as a sequencer around its five judgement dispatches, proportionate to the epic — a staged design is reused when the spec index reads REUSE; a plan is reused when an `AUDIT: PASS` is on record for this task set (fingerprint in the note) and the spec index reads REUSE; under `plan_tiers` the architect runs at strong unless `models/complexity.py`'s card (noted on the epic) flags the surface, and the audit at worker when it reads simple, each free to `ESCALATE` once to its declared tier: spec-index status → survey (full / DELTA + stamp / reuse) with the `ADEQUACY:` note and the staged spec index → the ABSENT path (draft, audit, or REQUIREMENT + park) → fold-in ① (register gate, spec-editor) → architect (`ADEQUACY: ABSENT` parks; `DECISION:` lines become tasks) → the design staged with draft decision records → planner (with `adr-next`) → analyst audit (a FAIL re-dispatches the planner fresh with the findings; a second FAIL parks) → the gate → `apply-plan.sh`. Exit 6 at each interactive approval with the `--from` that continues; 4 parked; 2 could not judge; on 0 and 4 the staging folder and the export are committed and pushed by the sequencer itself. `--detach` runs it through `fanout` (it outlives one Bash call) and `--wait` exits 5 while it runs |
| `swarm/stage-design.sh <epic> <result file>` | `/design`'s staging: `design.md` under the epic's folder (found or created by the close-out's rule), a decision task and a draft record per `DECISION:` line, the `ARCHITECTURE:` pointer |
| `tracker/tk.sh adr-next` | the next free decision-record number in `paths.adrs` — allocated once, before the planner runs, never by a worker |
| `tracker/tk.sh validate <epic> --paths` | the dependency waves plus the file-contention edges they cannot see: every path two tasks of one wave both name (existing, with its line count and the megafile flag; or one both would create). The planner resolves each edge; `apply-plan.sh` prints them after the write |
| `swarm/probe-worktree.sh [<lane>] [--worker N] [--keep]` | `/harness-setup` §7 as one call: a detached scratch worktree where the dispatcher puts a worker's, the init run inside it, `.swarm-env` parsed and SOURCED in a fresh shell, `TRACKER_ACTOR`, `SWARM_LANE` and every per-worker stack variable checked, the worktree removed whatever happened. Exit 1 names the failing line |
| `swarm/staged.sh section <file> "<heading>"` / `set-status <file> <status> [k=v…]` / `promote-adr <draft> --decision "…"` | the spec-editor's three staged-folder edits, line-anchored: the body under a heading (exit 1 absent, 2 duplicated — never a match inside prose), a frontmatter flip in place with `folded_in` dated, and a resolved draft decision record `git mv`d into `paths.adrs` at `adr-next` with its status and decision filled in |
| `wavelab/drive-interactive.py <cwd> <seconds> <prompts> [claude args]` | the lab's only way to exercise an interactive command: a real session in a pty, its dialogs answered (folder trust, a Bash permission ask, an `AskUserQuestion`'s first option). Read the transcript and `session-cost.sh`, never the screen |
| `swarm/doctrine.sh <agent>` | the agent's doctrine — every skill its definition declares, verbatim, the text the dispatcher puts in its system prompt — written to `.harness/run/doctrine/<agent>.md` for an agent-teams teammate, which loads none of it itself; refuses an agent the plugin does not ship |
| `checks/check-project-config.sh --stamp` | writes `harness.version` into `harness.yaml` from the plugin manifest, in place, then runs the check — the stamp is never typed |
