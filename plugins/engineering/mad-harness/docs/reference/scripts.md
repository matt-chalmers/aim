...
| `swarm/plan-epic.sh <epic> [--mode auto\|interactive] [--from <stage>] [--triage …] [--reset] [--no-push] [--detach]` / `--wait <run-id> [--timeout 540]` | campaign-loop §3 as a sequencer around its five judgement dispatches: spec-index status → survey (full / DELTA + stamp / reuse) with the `ADEQUACY:` note and the staged spec index → the ABSENT path (draft, audit, or REQUIREMENT + park) → fold-in ① (register gate, spec-editor) → architect (`ADEQUACY: ABSENT` parks; `DECISION:` lines become tasks) → the design staged with draft decision records → planner (with `adr-next`) → analyst audit (a FAIL re-dispatches the planner fresh with the findings; a second FAIL parks) → the gate → `apply-plan.sh`. Exit 6 at each interactive approval with the `--from` that continues; 4 parked; 2 could not judge; on 0 and 4 the staging folder and the export are committed and pushed by the sequencer itself. `--detach` runs it through `fanout` (it outlives one Bash call) and `--wait` exits 5 while it runs |
