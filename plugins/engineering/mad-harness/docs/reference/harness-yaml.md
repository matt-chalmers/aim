...
| `tiers` | the tier DEFINITIONS, patched over the plugin's: per tier, any of `provider`, `model`, `effort`, `max_budget_usd`, `price` (**required off Anthropic** — the CLI prices a third-party endpoint from its own table; measured at $5.00/Mtok for DeepSeek against $0.66–1.32 published — see [`models/pricing.py`](../../harness/models/pricing.py)) — keys you leave out are the plugin's, so an upgrade still reaches you; `model` and `provider` move together — see [`models/resolve.py`](../../harness/models/resolve.py) `merge_model_config` | no |
| `providers` | the provider set, patched per name and, within one, `env` per key; `billing: subscription\|metered` says which pocket a dispatch spends from (both real, reported apart); a credential must be a `${VAR}` reference (`harness.yaml` is committed) — same module | no |
