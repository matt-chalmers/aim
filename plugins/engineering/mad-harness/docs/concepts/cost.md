...
| `preload_declared` | +61% cost per run ($2.35 → $3.78), spreads apart — but the arm that carried the doctrine ran mutation testing in 22% of sessions against 6%, and a headless epic without it failed L2 for decorative assertions: the cost is mostly the doctrine being *followed* (~$0.16 of the +$0.59 per dispatch is carriage; the rest is 18 more turns of work). A cost-only A/B cannot judge this lever; the rig now runs the lenses (`--lenses`) and reports first-pass PASS rates per arm | **on** — behaviour first, until the fidelity series decides | 0.10.17 |
