...
| `plan_tiers` | first series (§3 alone, READY lab epic, 2 clean runs/arm): sanity-check at strong vs strategic −27% cost, −28% time, −42% output (only the output spread separates); a series with the planner and audit under it is in the 0.10.28 note | **on** — the second exception: moved on the owner's decision, not a separated spread | 0.10.28 |
