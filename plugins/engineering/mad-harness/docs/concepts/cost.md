...
| `plan_tiers` | first series (§3 alone, READY lab epic, 2 clean runs/arm): sanity-check at strong vs strategic −27% cost, −28% time, −42% output (only the output spread separates). Pending: a series with the planner and audit under it | off | 0.10.28 |
